...
export class CesiumVehicleGame {
  ...
  public async startCinematicSequence(): Promise<void> {
    ...
    // Use a temporary high position for the zoom animation
    const tempPosition = Cesium.Cartesian3.fromDegrees(base.longitude, base.latitude, 5000);

    this.scene.startEarthSpin();
    await this.delay(3000);

    this.scene.stopEarthSpin();
    await this.scene.zoomToLocation(tempPosition, 4500);

    // After tiles have loaded, query actual ground height and spawn 500m above it
    let spawnAlt = 3000; // fallback
    const groundPos = this.scene.clampToHeight(
      Cesium.Cartesian3.fromDegrees(base.longitude, base.latitude, 10000)
    );
    if (groundPos) {
      const groundHeight = Cesium.Cartographic.fromCartesian(groundPos).height;
      spawnAlt = groundHeight + 500;
      console.log(`Ground height: ${Math.round(groundHeight)}m, spawning at ${Math.round(spawnAlt)}m`);
    }

    const spawnPosition = Cesium.Cartesian3.fromDegrees(base.longitude, base.latitude, spawnAlt);
    const aircraft = await this.vehicleManager.spawnAircraft('aircraft', spawnPosition, base.heading);
